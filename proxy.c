		...
		for (loop = 0; loop < 2; loop++) {
			...
			*/
			...
			if (!loop && data[0]->req && !authok) {
				errdata = NULL;
                                if (use_spnego)
                                    i = authenticate_spnego(*wsocket[0], data[0], &closed);
                                else
                                    i = authenticate_spnego(*wsocket[0], data[0], tcreds, &closed);
				if (!i)
	...
	while (!quit) {
		if (plist_count(connection_list) < precache && check_active() < 1) {
			...
			data1->http = strdup("1");
                        ...
                        if (use_spnego)
                            i = authenticate_spnego(sd, data1, &closed);
                        else
                            i = authenticate_ntlm(sd, data1, creds, &closed);
	...
	for (i = 0; i < MAGIC_TESTS; ++i) {
		...
		if (nc <= 0) {
			...
			return;
                ...
                if (use_spnego)
                    c = authenticate_spnego(nc, req, &closed);
                else
                    c = authenticate_ntlm(nc, req, creds, &closed);
		if (c <= 0 || c == 500 || closed) {
...
int main(int argc, char **argv) {
	char *tmp, *head;
	char *cpassword, *cpassntlm2, *cpassnt, *cpasslm, *cuser, *cdomain, *cworkstation, *cuid, *cpidfile, *cauth, *cspn;
	struct passwd *pw;
	...
	if (cf) {
		...
		free(tmp);
                 ...
                 */
                tmp = new(MINIBUF_SIZE);
                CFG_DEFAULT(cf, "UseSPNEGO", tmp, MINIBUF_SIZE);
                if (!strcasecmp("yes", tmp)) {
                    ntlmbasic = 0;
                    use_spnego = 1;
                }
		...
		CFG_DEFAULT(cf, "ServicePrincipalName", cspn, MINIBUF_SIZE);
	 ...
	 */
	if ((interactivehash || (interactivepwd && !ntlmbasic)) && !use_spnego) {
		printf("Password: ");
		tcgetattr(0, &termold);
		termnew = termold;
	 ...
	 */
	if (!use_spnego && !ntlmbasic && ((creds->hashnt && !creds->passnt) || (creds->hashlm && !creds->passlm) || (creds->hashntlm2 && !creds->passntlm2))) {
